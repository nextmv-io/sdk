...
def main() -> None:
    ...
    parser.add_argument(
        "-reduction-factor",
        default=0.0,
        help="Reduction factor to reduce the demand by.",
        type=float,
    )
    ...
    solution = solve(
        input_data, args.duration, args.include_past, args.reduction_factor
    )
...
def solve(
    ...
    # Use a custom bigM, since solver.infinity() causes abnormal termination on platform.
    bigm = 100000000
    ...
    if status == pywraplp.Solver.OPTIMAL:
        ...
        log(f"No optimal solution found. Status: {status}")

    solution = {
        "forecast": demands + forecast if include_past else forecast,
    }

    if reduction_factor > 0:
        for i in solution["forecast"]:
            if "count" in i:
                i["count"] = int(math.ceil(i["count"] * reduction_factor))
